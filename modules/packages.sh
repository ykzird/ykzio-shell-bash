...
manage_required_packages() {
    ...
    if [[ $LOG_LEVEL -eq $LOG_LEVEL_DEBUG ]]; then
        ...
        gum spin --spinner dot --title "Updating package lists" -- sudo apt-get update || true
    fi
    ...
    for pkg in "${missing_packages[@]}"; do
        if [[ $LOG_LEVEL -eq $LOG_LEVEL_DEBUG ]]; then
            wait_for_apt_lock
            gum spin --spinner dot --title "Installing $pkg" -- bash -c "wait_for_apt_lock && sudo apt-get install -y "$pkg""
        else
